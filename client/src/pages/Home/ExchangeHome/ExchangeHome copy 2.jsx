...
export const ExchangeHome = (props) => {
  ...
  useEffect(() => {
    if (exchangeRateInfo === '0.000') {
      // oneCylce = duration + delay;
      setActiveInterval(70000);

      setTimeout(() => {
        setActiveInterval(10000);
      }, 70000);
    }
  }, [exchangeRateInfo]);
  ...
  useEffect(() => {
    ...
    // let duration = 30000; // stable mode // 15000 for buy and sell since we're only making a single request per time
    // let delay = 60000; //stable mode

    // let duration = 10000;
    let duration = nextInterval;
    ...
    // if (retryMessage) {
    //   priceInterval = setInterval(fetchPrices, duration + delay); // once every 30 seconds (i.e 4 calls per minute)
    // }

    // if (Number(exchangeRateInfo) === 0) {
    //   // oneCylce = duration + delay;
    //   // setActiveInterval(duration + delay);

    //   priceInterval = setInterval(fetchPrices, duration); // once every 30 seconds (i.e 4 calls per minute)
    // }

    // if (Number(exchangeRateInfo) > 0) {
    //   setActiveInterval(duration);
    ...
    //   priceInterval = setInterval(fetchPrices, oneCylce); // once every 30 seconds (i.e 4 calls per minute)
    // }
    priceInterval = setInterval(fetchPrices, duration); // once every 30 seconds (i.e 4 calls per minute)
  ...
  // useEffect(() => {
  //   if (Number(fValue) > 0 && Number(tValue) === 0 && exchangeRate) {
  //     setLoading(true);
  //     setTimeout(() => {
  //       priceDataException();
  //     }, 30000); // 30 seconds interval due to api rate limits
